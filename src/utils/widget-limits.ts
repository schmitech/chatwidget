import type { WidgetRuntimeConfig } from '../types/widget.types';
...
export const getWidgetLimits = () => {
  ...
  if (typeof window !== 'undefined' && window.ChatbotWidget?.getCurrentConfig) {
    try {
      const config = window.ChatbotWidget.getCurrentConfig() as WidgetRuntimeConfig | undefined;
      const chatConstants = config?.CHAT_CONSTANTS;
      if (chatConstants) {
        return {
          MAX_SUGGESTED_QUESTION_LENGTH: chatConstants.MAX_SUGGESTED_QUESTION_LENGTH || WIDGET_LIMITS.MAX_SUGGESTED_QUESTION_LENGTH,
          MAX_SUGGESTED_QUESTION_QUERY_LENGTH: chatConstants.MAX_SUGGESTED_QUESTION_QUERY_LENGTH || WIDGET_LIMITS.MAX_SUGGESTED_QUESTION_QUERY_LENGTH,
